...
#endif
//static irq_handle_t g_irq_handle = NULL;
    ...
    switch (p_cellular_receive->data)
    {
        ...
        case CHAR_CHECK_4:
        {
                                ...
                                ATC_RES_OK))
            {
                sscanf_ret = sscanf((char *) //(uint8_t *)->(char *)
                                &p_ctrl->sci_ctrl.receive_buff[p_cellular_receive->tmp_recvcnt],
                                    "%d.%d.%d.%d", &dns_1, &dns_2, &dns_3, &dns_4);
                if (4 == sscanf_ret)
                {
        ...
        case CHAR_CHECK_4:
        {
                                ...
                                ATC_RES_BEGIN_OR_END))
            {
                sscanf((char *) //(uint8_t *)->(char *)
                    &p_ctrl->sci_ctrl.receive_buff[p_cellular_receive->tmp_recvcnt],
                        " %d" , &p_cellular_receive->socket_no);

                p_ctrl->p_socket_ctrl[p_cellular_receive->socket_no - CELLULAR_START_SOCKET_NUMBER].socket_status
        ...
        case CHAR_CHECK_4:
        {
                                ...
                                ATC_RES_BEGIN_OR_END))
            {
                ...
                if ((0 == ind) && (0 == value))
                {
                    ...
                    //R_IRQ_Open(IRQ_NUM_4, IRQ_TRIG_FALLING, IRQ_PRI_1, &s_irq_handle, irq_callback);
                }
                else
                {
