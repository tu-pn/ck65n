                            ...
                            const uint32_t ip_address, const uint16_t port)
{
    ...
    if (p_ctrl->sci_ctrl.atc_timeout >
        ((p_ctrl->p_socket_ctrl[socket_no - CELLULAR_START_SOCKET_NUMBER].connect_timeout * 100) + CELLULAR_SOCKETCONNECT_DELAY))
    {
    ...
    else
    {
        at_ret = cellular_execute_at_command(p_ctrl,
                    ((uint32_t)p_ctrl->p_socket_ctrl[socket_no - CELLULAR_START_SOCKET_NUMBER].connect_timeout * 100) + CELLULAR_SOCKETCONNECT_DELAY,
                        ATC_RETURN_OK, ATC_CONNECT_SOCKET);
    }
